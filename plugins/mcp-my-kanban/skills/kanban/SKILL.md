...
## ⛔ 승인 전 작업 절대 금지 — 가장 중요한 규칙

**태스크를 `task_create`로 등록한 후, 사용자가 웹 UI에서 승인할 때까지 해당 태스크의 구현 작업을 절대 시작하지 않는다.**

이것은 이 MCP 서버의 존재 이유다. 사용자가 작업 범위를 검토하고 승인하는 게이트가 없으면 칸반보드를 쓸 이유가 없다.

### 금지 행위 (승인 전)

| 금지 | 이유 |
|------|------|
| 코드 수정/생성 | 승인 없는 작업은 사용자가 원하지 않는 변경일 수 있다 |
| 파일 탐색/분석 후 구현 시작 | 탐색은 허용되나, 구현은 승인 후에만 |
| "먼저 해놓고 리뷰 받겠다" | 이것이 바로 금지하는 행위 그 자체다 |
| `task_start` 호출 | 서버가 거부한다 — `todo` 상태에서는 시작 불가 |

### 올바른 흐름

```
에이전트: task_create로 태스크 등록
         ↓
         ⏸️ 정지. 사용자 승인을 기다린다.
         사용자에게 "태스크를 등록했습니다. 웹 UI에서 승인해주세요." 안내.
         ↓
사용자:   웹 UI에서 태스크 승인 (todo → approved)
         ↓
에이전트: task_list status=approved 로 승인 확인
         ↓
에이전트: task_start로 작업 시작
```
...
| 합리화 | 현실 |
|--------|------|
| "간단한 작업이니까 바로 해도 된다" | 간단해도 승인이 필요하다. 규칙에 예외 없다. |
| "어차피 승인될 테니 미리 시작하자" | 사용자가 거부하거나 수정할 수 있다. 기다려라. |
| "태스크 등록과 동시에 진행하면 효율적이다" | 효율이 아니라 월권이다. 사용자가 통제권을 잃는다. |
| "탐색만 하는 거니까 괜찮다" | 탐색은 괜찮지만, 파일 수정은 안 된다. |
| "사용자가 빨리 하라고 했다" | 빨리 = 승인 후 빠르게. 승인 없이가 아니다. |
...
**Notes:**
- `todo`: 사용자 미승인 상태. **에이전트는 이 상태의 태스크를 절대 작업할 수 없다.** 승인을 기다려야 한다.
- `approved`: 사용자가 웹 UI에서 승인한 상태. 이때부터 에이전트가 `task_start`로 작업을 시작할 수 있다.
...
1. **승인된 작업 확인** — `approved` 상태인 태스크만 작업 대상이다
   ```
   task_list status=approved
   ```
   > ⚠️ 승인된 태스크가 없으면 **작업하지 않는다.** 사용자에게 승인을 요청하고 기다린다.
